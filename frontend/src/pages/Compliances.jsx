      ...
      {overdue.length > 0 && (
        <div className="card p-4 border-l-4 border-red-500 bg-red-50 dark:bg-red-900/10">
          <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
            ...
            <span className="font-medium">{overdue.length} {t.overdueCompliance}</span>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800/50 text-left">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">{t.mine}</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                <tr><td colSpan="5" className="px-4 py-12 text-center text-slate-400">{t.loading}</td></tr>
              ) : compliances.length === 0 ? (
                <tr><td colSpan="5" className="px-4 py-12 text-center text-slate-400">{t.noCompliances}</td></tr>
              ) : (
                compliances.map((c) => (
